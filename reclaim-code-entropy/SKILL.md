...
description: Reclaim code entropy — find, rank, and safely remove accidental complexity. Use when asked to simplify or clean up code, remove dead or over-engineered surface, or collapse duplicate state or APIs.
---
...
Preserve validation at trust boundaries, authorization, security controls, accessibility basics, data-loss prevention, durable-data compatibility, and cleanup that establishes resource quiescence.

## Survey For Entropy

Start with large or central production surfaces, not only obvious unused symbols. Run installed dead-code or dependency tools when useful, but treat every result as a lead.
...
Distinguish duplication from necessary independence: separate backends, adapters, representations, or lifecycle mechanisms may intentionally test a contract or protect distinct owners.
...
- The change is a product decision, not cleanup.
...
6. Add compatibility shims only for a real compatibility obligation; when one exists, keep the path or design an explicit migration instead of silently deleting it.
...
Ground value claims in net maintenance reduction, not green tests or deletion volume alone.
